...
public class AdminPanel extends JFrame {
    ...
    public AdminPanel() {
        ...
        setUndecorated(true); // Removes the top window bar, preventing resizing or closing via the 'X'
        setExtendedState(JFrame.MAXIMIZED_BOTH); // Forces the app to cover the entire monitor
        // ----------------------------------
    ...
    private void generatePDFReport(String query, String[] params, String fileName, String title) {
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            // ---------------------------------------------------------
            // NEW FEATURE: AUTO-OPEN REPORT POP-UP
            // ---------------------------------------------------------
            try {
                // Target the exact file path we just passed to the generator
                java.io.File pdfFile = new java.io.File(fileName);
                
                // If the computer supports opening files, launch the PDF!
